...
public StartMoveLine(playerid)
{
	...
	switch(el_line_state)
	{
		...
		case 2: // передвигаем
		{
		    if(IsPlayerConnected(playerid)) PlayerPlaySound(playerid, 17801, 0.0, 0.0, 0.0);
		    el_line_state--;
		    MoveLineLR(-12.5);
		}
		case 1: //опускаем
...
public EL_AddObject()
{
		for(new i = sizeof(el_boxes)-1; 0 <= i; i--)
		{
		    if(IsValidObject(el_boxes[i]))
			{
			    	SendClientMessageToAll(0xff8800FF, " Новогодняя игрушка лежит на прелавке, скорее несите к новогодней ёлке!");
